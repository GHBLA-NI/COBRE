...
write.csv(df_adjusted_reor,"../output/df_adjusted_reor.csv")                                  
                
                                  
                                  
                                  
bim <- read.table("../data/PE_maternal_09_10_control_region_filtered.bim", header = FALSE,col.names = c("CHR", "SNP", "cm", "BP","allele1", "allele2"))

df_adjusted_reor<-read.csv("../output/df_adjusted_reor.csv",row.names = 1)
...
target_snps <- snplist
selected_snps <- bim[bim$SNP %in% target_snps, c("SNP", "BP")]
